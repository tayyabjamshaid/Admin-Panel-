...
import { fetchPrivateGroups } from "Reducers/fetchPrivateGroups";
...
function PublicGroup() {
  ...
  const { privateGroups, error } = useSelector(
    (state) => state.allPrivateGroups
  );

  useEffect(() => {
    dispatch(fetchPrivateGroups());
  }, [dispatch]);
  const userArrayMethod = useMemo(() => {
    setUserArray(privateGroups);
  ...
  }, [userArray, currentPage, search, privateGroups, ITEMS_PER_PAGE]);
      ...
      <div className="content">
        ...
        <Row>
          ...
          <Col md="12">
            <Card>
              <CardHeader>
                <CardTitle tag="h4">Private Groups</CardTitle>
              </CardHeader>
              <CardBody>
                      ...
                      userArrayMethod.map((userAllData, i) => {
                        return (
                          <tr
                            ...
                            onClick={() => {
                              history.push(
                                `/admin/groupDetail/${userAllData.group_id}/private`
                              );
                            }}
                          >
                            <td>
